...
class RandomTokenMask(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        mask = torch.rand(batch, length, device=tokens.device) < ratio
...
class ImageHiddenDropout(nn.Module):
    def __init__(
        self,
    ...
    def _mask_token(self, dim: int, ref: torch.Tensor) -> torch.Tensor:
        ...
        if self.mask_token is None:
            self.mask_token = nn.Parameter(torch.zeros(1, 1, dim, device=ref.device, dtype=ref.dtype))
        if self.mask_token.shape[-1] != dim:
            raise ValueError(f"image dropout mask dim {self.mask_token.shape[-1]} does not match input dim {dim}.")
        return self.mask_token.to(device=ref.device, dtype=ref.dtype)
    ...
    def forward(
        ...
        *,
        ...
        if tokens.ndim != 3:
            raise ValueError(f"tokens must have shape [B, N, D], got {tuple(tokens.shape)}.")
        active = self.training if training is None else bool(training)
        prob = self.dropout_prob if dropout_prob is None else float(dropout_prob)
        if not 0.0 <= prob <= 1.0:
            raise ValueError("dropout_prob must be in [0, 1].")
        if not active or prob <= 0.0:
            return tokens, {
                "image_dropout_active": tokens.new_zeros(()),
                "image_dropout_prob": tokens.new_tensor(float(prob)),
            }
        batch, length, dim = tokens.shape
        drop = torch.rand(batch, device=tokens.device) < prob
        replacement = self._mask_token(dim, tokens).expand(batch, length, dim)
        out = torch.where(drop.view(batch, 1, 1), replacement, tokens)
        return out, {
            "image_dropout_active": drop.float().mean().to(dtype=tokens.dtype),
            "image_dropout_prob": tokens.new_tensor(float(prob)),
        }
...
class JEPADynamicAdapter(nn.Module):
    def __init__(
        ...
        self.image_dropout = ImageHiddenDropout(dropout_prob=image_dropout_prob, token_dim=vlm_hidden_dim)
    ...
    def forward(
        ...
        *,
        use_image_memory: bool = True,
        image_mask_ratio: Optional[float] = None,
        image_dropout_prob: Optional[float] = None,
    ) -> Dict[str, Any]:
        if use_image_memory and (image_hidden is None or image_hidden.ndim != 3):
        ...
        batch = int(h_dyn.shape[0])
        zero = h_dyn.new_zeros(())
        image_tokens: Optional[torch.Tensor] = None
        dropout_info = {"image_dropout_active": zero, "image_dropout_prob": zero}
        mask_info = {"actual_mask_ratio": zero}
        if use_image_memory:
            assert image_hidden is not None
            dropped_image, dropout_info = self.image_dropout(
                image_hidden,
                training=self.training,
                dropout_prob=image_dropout_prob,
            )
            masked_image, mask_info = self.mask(dropped_image, training=self.training, mask_ratio=image_mask_ratio)
            image_tokens = self.image_proj(masked_image)
        preds = []
        group_losses = []
        for group_idx, decoder in enumerate(self.group_decoders):
            slot_tokens = self.h_dyn_proj(h_dyn[:, group_idx])
            memory = slot_tokens if image_tokens is None else torch.cat([image_tokens, slot_tokens], dim=1)
        ...
        diagnostics = {
            ...
            "dynamic_image_dropout_active": dropout_info["image_dropout_active"].detach(),
            "dynamic_use_image_memory": pred.new_tensor(1.0 if use_image_memory else 0.0),
        }
...
class VGGTFeature23Adapter(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if (use_image_memory and (image_hidden is None or image_hidden.ndim != 3)) or h_geo.ndim != 3:
        ...
        if use_image_memory:
            ...
            masked_image, mask_info = self.mask(dropped_image, training=self.training, mask_ratio=image_mask_ratio)
            image_tokens = self.image_proj(masked_image)
        geo_tokens = self.h_geo_proj(h_geo)
        memory = geo_tokens if image_tokens is None else torch.cat([image_tokens, geo_tokens], dim=1)
        ...
        return {
            ...
            "diagnostics": {
                ...
                "geometry_image_dropout_active": dropout_info["image_dropout_active"].detach(),
                "geometry_use_image_memory": pred.new_tensor(1.0 if use_image_memory else 0.0),
            },
        }
...
class TwoExpertTrajectoryProbe(nn.Module):
    def __init__(
        ...
        self.context_dim = 8 + 3 + 12
        fused_dim = adapter_dim * 2 + self.context_dim
        branch_dim = adapter_dim + self.context_dim
        ...
        self.dyn_head = nn.Sequential(
            ...
            nn.Linear(branch_dim, adapter_dim * 2),
            nn.GELU(),
            nn.Linear(adapter_dim * 2, action_horizon * action_dim),
        )
    ...
    def forward(
        ...
        *,
        ...
        context = torch.cat([status, command, history_vec], dim=-1)
        pred = self.fused_head(torch.cat([dyn_summary, geo_summary, context], dim=-1)).view(
            batch,
            self.action_horizon,
            self.action_dim,
        )
        pred_dyn = self.dyn_head(torch.cat([dyn_summary, context], dim=-1)).view(batch, self.action_horizon, self.action_dim)
        pred_geo = self.geo_head(torch.cat([geo_summary, context], dim=-1)).view(batch, self.action_horizon, self.action_dim)
        zero = pred.new_zeros(())
        losses = {
            "probe_loss": zero,
            "probe_dyn_loss": zero,
            "probe_geo_loss": zero,
            "probe_heading_loss": zero,
            "probe_progress_loss": zero,
            "geo_lateral_profile_loss": zero,
            "geo_heading_profile_loss": zero,
        }
        if target_action_norm is not None:
            ...
            losses["probe_dyn_loss"] = F.smooth_l1_loss(pred_dyn.float(), target.detach().float()).to(dtype=pred.dtype)
            losses["probe_geo_loss"] = F.smooth_l1_loss(pred_geo.float(), target.detach().float()).to(dtype=pred.dtype)
            ...
            losses["geo_lateral_profile_loss"] = F.smooth_l1_loss(pred_geo[..., 1].float(), target.detach()[..., 1].float()).to(dtype=pred.dtype)
            losses["geo_heading_profile_loss"] = F.smooth_l1_loss(pred_geo[..., 2].float(), target.detach()[..., 2].float()).to(dtype=pred.dtype)
        return {
            "probe_traj_norm": pred,
            "probe_dyn_traj_norm": pred_dyn,
            "probe_geo_traj_norm": pred_geo,
            "losses": losses,
            "diagnostics": {
                "probe_traj_norm": pred.detach().float().norm(dim=-1).mean().to(dtype=pred.dtype),
                "probe_dyn_traj_norm": pred_dyn.detach().float().norm(dim=-1).mean().to(dtype=pred.dtype),
                "probe_geo_traj_norm": pred_geo.detach().float().norm(dim=-1).mean().to(dtype=pred.dtype),
            },
        }
