...
def _run_dataloader_inference(
    ...
    # Create the iterator before moving the model to CUDA so worker processes do
    # not inherit a live CUDA context when Linux fork workers are used.
    ...
    for features, token_batch, batch_failures in tqdm(data_iter, total=batch_total, desc="Generating DDV2 submission"):
