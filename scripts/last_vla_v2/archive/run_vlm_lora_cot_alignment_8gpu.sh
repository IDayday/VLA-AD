...
LORA_VISION_LAST_N="${LORA_VISION_LAST_N:-0}"
LORA_HIDDEN_ANCHOR_WEIGHT="${LORA_HIDDEN_ANCHOR_WEIGHT:-0.01}"
LORA_HIDDEN_ANCHOR_MODE="${LORA_HIDDEN_ANCHOR_MODE:-summary_cosine}"
...
cmd=(
  ...
  agent.last_vla_vlm_lora_init="${LORA_INIT}"
  agent.last_vla_vlm_lora_vision_last_n="${LORA_VISION_LAST_N}"
  agent.lr_vlm_lora="${LR_VLM_LORA:-1e-5}"
  agent.lr_last_vla_cot="${LR_LAST_VLA_COT:-1e-4}"
  agent.weight_decay_vlm_lora="${WEIGHT_DECAY_VLM_LORA:-0.0}"
  agent.weight_decay_last_vla_cot="${WEIGHT_DECAY_LAST_VLA_COT:-1e-4}"
  agent.last_vla_hidden_anchor_weight="${LORA_HIDDEN_ANCHOR_WEIGHT}"
  agent.last_vla_hidden_anchor_mode="${LORA_HIDDEN_ANCHOR_MODE}"
...

{
  ...
  printf 'LORA_PRESET=%s\n' "${LORA_PRESET}"
  printf 'LORA_SCOPE=%s\n' "${LORA_SCOPE}"
  printf 'LORA_R=%s\n' "${LORA_R}"
  printf 'LORA_ALPHA=%s\n' "${LORA_ALPHA}"
  printf 'LORA_DROPOUT=%s\n' "${LORA_DROPOUT}"
  printf 'LORA_USE_RSLORA=%s\n' "${LORA_USE_RSLORA}"
  printf '%q ' "${cmd[@]}"
  printf '\n'
} >>"${COMMANDS_LOG}"
