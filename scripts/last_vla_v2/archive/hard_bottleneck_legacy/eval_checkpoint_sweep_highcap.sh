...
EVAL_ROOT="${OUT_ROOT}/highcap_no_risk_eval"
...
for line in serverA_frozen_highcap_no_risk serverB_lora_highcap_no_risk; do
  ...
  if [[ "${line}" == "serverB_lora_highcap_no_risk" ]]; then
    if [[ -z "${LORA_NAVTEST_CHUNK_CACHE_ROOT:-}" ]]; then
      echo "Skipping serverB_lora_highcap_no_risk eval commands: LORA_NAVTEST_CHUNK_CACHE_ROOT is required." >&2
      continue
    fi
