...
required=(OUTPUT_DIR MASTER_PORT VLM_PATH TEACHER_CACHE_ROOT RECOGDRIVE_STAGE1_REPLAY_CACHE)
...
TRAIN_LOG="${OUTPUT_DIR}/logs/stage1_v2_clean.train.log"
...
status=$?
set -e
echo "stage1_v2_clean exited with status ${status}" >>"${COMMANDS_LOG}"
exit "${status}"
