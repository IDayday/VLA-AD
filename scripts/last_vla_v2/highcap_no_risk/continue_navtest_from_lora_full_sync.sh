...
OUT_ROOT="${OUT_ROOT:-/mnt/project/VLA-AD/outputs/last_vla_v2/highcap_no_risk_navtest_eval_20260606T034658Z}"
...
NUM_GPUS="${NUM_GPUS:-8}"
LORA_PROCS_PER_GPU="${LORA_PROCS_PER_GPU:-8}"
...
COMMANDS_LOG="${OUT_ROOT}/commands.continue_lora_full_sync_${RUN_ID}.log"
PROGRESS_LOG="${OUT_ROOT}/progress.continue_lora_full_sync_${RUN_ID}.log"
LORA_LOG_DIR="${LOG_DIR}/lora_navtest_retry_${RUN_ID}"
EVAL_LOG_DIR="${LOG_DIR}/eval_full_sync_${RUN_ID}"
...
require_path "${A_NAVTEST_CACHE}" "completed A navtest highcap cache"

log "out_root=${OUT_ROOT}"
log "continuing from completed A navtest cache; rebuilding B LoRA cache only"
...
if [[ "${SKIP_A_AUDIT:-0}" == "1" && -s "${OUT_ROOT}/A_navtest_highcap_manifest.json" ]]; then
  log "skipping A audit; using existing ${OUT_ROOT}/A_navtest_highcap_manifest.json"
else
  log "auditing existing A navtest highcap cache"
  cmd=(
    ...
    --strict-no-risk
    --output "${OUT_ROOT}/A_navtest_highcap_manifest.continue_${RUN_ID}.json"
  )
  quote_cmd "${cmd[@]}"
  "${cmd[@]}" >"${LOG_DIR}/audit_A_navtest_highcap_continue_${RUN_ID}.log" 2>&1
fi

log "removing failed LoRA navtest cache dirs from previous attempt"
rm -rf "${B_LORA_SHARD_ROOT}" "${B_NAVTEST_CACHE}" "${CACHE_ROOT}/lora_smoke_after_peft"

log "building B LoRA navtest hidden cache with ${LORA_NUM_SHARDS} shards (${LORA_PROCS_PER_GPU}/gpu)"
mkdir -p "${B_LORA_SHARD_ROOT}" "${B_NAVTEST_CACHE}/samples" "${LORA_LOG_DIR}"
...
for shard in $(seq 0 $((LORA_NUM_SHARDS - 1))); do
  ...
  shard_log="${LORA_LOG_DIR}/${shard_name}.log"
  mkdir -p "${shard_dir}"
  cmd=(
...
"${PYTHON_BIN}" - "${B_LORA_SHARD_ROOT}" "${B_NAVTEST_CACHE}" "${EXPECTED_NAVTEST_SAMPLES}" "${LORA_NUM_SHARDS}" <<'PY' >"${LOG_DIR}/merge_B_lora_navtest_continue_${RUN_ID}.log" 2>&1
...
if len(shard_dirs) != num_shards:
    raise RuntimeError(f"Found {len(shard_dirs)} shard dirs, expected {num_shards}.")
...
cmd=(
  ...
  --geometry-teacher-dim 512
  --strict-no-risk
  --output "${OUT_ROOT}/B_lora_navtest_highcap_manifest.continue_${RUN_ID}.json"
)
quote_cmd "${cmd[@]}"
"${cmd[@]}" >"${LOG_DIR}/audit_B_lora_navtest_highcap_continue_${RUN_ID}.log" 2>&1
...
def checkpoint_label(src: Path) -> str:
    if "lightning_logs" in src.parts:
        idx = src.parts.index("lightning_logs")
        version = src.parts[idx + 1] if idx + 1 < len(src.parts) else "version_unknown"
        return f"lightning_{version}_{src.name}"
    return f"top_{src.name}"
...
with manifest_path.open("w", encoding="utf-8") as f:
    for row in rows:
        f.write(json.dumps(row, sort_keys=True) + "\n")
(manifest_path.parent / "eval_manifest_skipped.json").write_text(
...
log "eval submitted jobs=${idx}; waiting for all full-sync jobs"
wait_stage "eval" "${stage_pids[@]}"

log "aggregating navtest eval metrics"
"${PYTHON_BIN}" - "${EVAL_ROOT}" "${OUT_ROOT}" <<'PY' >"${LOG_DIR}/aggregate_eval_continue_${RUN_ID}.log" 2>&1
from __future__ import annotations
