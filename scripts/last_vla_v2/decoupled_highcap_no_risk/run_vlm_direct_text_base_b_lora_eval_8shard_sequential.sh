...
OUT_ROOT=${OUT_ROOT:-/mnt/project/VLA-AD/outputs/last_vla_v2/direct_text_navtest_eval_8shard_seq_$(date -u +%Y%m%dT%H%M%SZ)}
...
echo "driver_pid=$$"
echo "mode=sequential_per_ckpt_8_shards"
...
for item in "${RUNS[@]}"; do
  ...
  echo "${run}" > "${OUT_ROOT}/active_run.txt"
  jobs_tsv="${OUT_ROOT}/jobs/${run}.tsv"
  : > "${jobs_tsv}"
  ...
  echo "[$(date -Is)] START run=${run} jobs=${jobs_tsv}"
  "${PYTHON}" "${LAUNCHER}" \
    --jobs-tsv "${jobs_tsv}" \
    --out-root "${OUT_ROOT}" \
    --stagger-seconds "${STAGGER_SECONDS}" \
    --poll-seconds "${POLL_SECONDS}"
  echo "[$(date -Is)] DONE run=${run}"
  completed_runs+=("${run}")
  "${PYTHON}" scripts/aggregate_vlm_direct_text_eval.py --out-root "${OUT_ROOT}" \
    $(printf -- '--run %q ' "${completed_runs[@]}") \
    > "${OUT_ROOT}/aggregate_${run}.log" 2>&1 || true
done

rm -f "${OUT_ROOT}/active_run.txt"
"${PYTHON}" scripts/aggregate_vlm_direct_text_eval.py --out-root "${OUT_ROOT}" \
