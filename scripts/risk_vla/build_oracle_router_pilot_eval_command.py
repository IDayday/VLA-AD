...
def eval_config(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    cfg.update(
        {
            "use_risk_vla": True,
            "risk_vla_use_oracle_router": True,
...
def build_eval_command(args: argparse.Namespace, snapshot_path: Path) -> List[str]:
    command = [
        ...
        "--output-dir",
        str(args.output_dir / "oracle_router_pilot_eval"),
        "--precision",
        args.precision,
    ]
...
def write_artifacts(args: argparse.Namespace) -> tuple[Path, Path, List[str]]:
    ...
    snapshot_path = args.output_dir / "oracle_router_pilot_config_snapshot.yaml"
    ...
    command_path = args.output_dir / "resolved_oracle_router_eval_command.sh"
    command_path.write_text("#!/usr/bin/env bash\nset -euo pipefail\n" + shlex.join(command) + "\n", encoding="utf-8")
    return snapshot_path, command_path, command


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or run the analysis-only oracle-router pilot PDM command.")
...
def main() -> int:
    ...
    print("WARNING: Oracle-router pilot is analysis-only. Do not use test labels for training.")
