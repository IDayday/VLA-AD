...
OUTPUT_DIR="${OUTPUT_DIR:-${BIT_EXP_ROOT}/critic/full}"
DRY_RUN=0
for arg in "$@"; do
...
cd "${PROJECT_ROOT}"
mkdir -p "${OUTPUT_DIR}"
if [[ -z "${CANDIDATE_NPZ:-}" || -z "${LABELS_JSONL:-}" ]]; then
  cat > "${OUTPUT_DIR}/CRITIC_BLOCKER.md" <<EOF
# Critic Training Blocker

Set CANDIDATE_NPZ and LABELS_JSONL to explicit >=${MIN_REAL_SAMPLES} train assets.
Navtest/test labels are forbidden for training.
EOF
  echo "Blocked: missing CANDIDATE_NPZ or LABELS_JSONL"
...
python scripts/risk_vla/train_trajectory_risk_critic.py "${args[@]}"
