...
RUN_NAME="${RUN_NAME:-stage3_rl_2b_safe_diffgrpo_navtest_eval_async_pdm_$(date -u +%Y%m%dT%H%M%SZ)}"
...
ASYNC_PDM_WORKERS="${ASYNC_PDM_WORKERS:-4}"
...
CMD=(
  ...
  "${REPO_ROOT}/navsim/planning/script/run_pdm_score_recogdrive_async_pdm.py"
  ...
  "experiment_name=stage3_safe_diffgrpo_eval_async_pdm"
