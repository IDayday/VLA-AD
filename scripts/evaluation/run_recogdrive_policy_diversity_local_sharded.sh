...
CHECKPOINT="${CHECKPOINT:?Set CHECKPOINT to the ReCogDrive checkpoint file to evaluate.}"
OUT_ROOT="${OUT_ROOT:?Set OUT_ROOT to the policy diversity output directory.}"
...
EVAL_TOKEN_FILE="${EVAL_TOKEN_FILE:-${ARTIFACT_ROOT}/outputs/stage3_policy_diversity_k8_navtrain10k_random_step21600_20260624T0630Z/navtrain_random10000_seed260306049_tokens.txt}"
...
LOCAL_EVAL_MASTER_PORT_BASE="${LOCAL_EVAL_MASTER_PORT_BASE:-29900}"
...
if [[ "${rc}" -ne 0 ]]; then
  log "one or more local policy diversity shards failed"
  for ((i = 0; i < LOCAL_EVAL_SHARD_COUNT; i++)); do
