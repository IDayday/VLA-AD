...
OUT_ROOT="${OUT_ROOT:?Set OUT_ROOT to the evaluation output directory.}"
...
run_shard() {
  ...
  local cmd=(
    ...
    "experiment_name=stage3_safe_diffgrpo_eval_exact_pool_pdm_local_shard_${shard_index}"
...
if [[ "${rc}" -ne 0 ]]; then
  log "one or more local eval shards failed"
  for ((i = 0; i < LOCAL_EVAL_SHARD_COUNT; i++)); do
...
log "local sharded eval complete aggregate_csv=${aggregate_csv}"
