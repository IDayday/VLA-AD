...
def _decision_for_current(
    ...
    best_seen_scenes = _int_or_none(row.get("best_seen_scenes"))
    best_epoch = _int_or_none(row.get("best_checkpoint_epoch"))
    ...
    if best_epoch is None and (
        best_seen_scenes is None or best_seen_scenes < int(full_comparison_min_seen_scenes)
    ):
        if pdms >= early_pdms_threshold:
            return "仅通过 early health gate；该 checkpoint 训练曝光不足，不能和长程强基线直接比较。"
        return "早期健康度偏弱；先等 matched exposure/epoch，或只和同 effective batch 的同 step 对照比较。"
    ...
    current_decisions = [
        {
            ...
            "decision": _decision_for_current(
                row,
...
def write_markdown(path: Path, report: dict, top_rows: list[dict[str, str]], baseline_pdms: float) -> None:
    ...
    lines.append("")
    lines.append(f"- Historical long-horizon Stage3/Safe DiffGRPO best PDMS: `{baseline_pdms:.6f}`")
    lines.append(f"- Original Stage3 epoch9-step13300 reference PDMS: `{report['original_stage3_pdms']:.6f}`")
    lines.append(f"- Early health gate PDMS: `{report['early_pdms_threshold']:.6f}`")
    lines.append(
        f"- Long-horizon baseline comparison requires comparable exposure; default threshold is about "
        f"`{report['full_comparison_min_seen_scenes']}` seen train scenes."
    )
    ...
    lines.append("")
    lines.append(_table_row(["run", "step", "seen", "eff batch", "ckpts", "reward", "safe", "eval rows", "best PDMS", "best seen", "decision"]))
    ...
    if stale_running:
        ...
        lines.append("")
        lines.append(_table_row(["run", "step", "seen", "event age sec", "ckpts", "best PDMS"]))
        lines.append(_table_row(["---", "---:", "---:", "---:", "---:", "---:"]))
    ...
    lines.append("")
    lines.append(_table_row(["family", "run", "ckpt", "seen", "PDMS", "delta vs long target", "DDC", "TTC", "EP"]))
    lines.append(_table_row(["---", "---", "---", "---:", "---:", "---:", "---:", "---:", "---:"]))
    ...
    lines.append("")
    lines.append(_table_row(["rank", "run", "family", "ckpt", "seen", "PDMS", "delta vs long target", "NC", "DAC", "TTC", "EP", "DDC"]))
    lines.append(_table_row(["---:", "---", "---", "---", "---:", "---:", "---:", "---:", "---:", "---:", "---:", "---:"]))
    ...
    lines.append("")
    lines.append("- Do not compare multi-GPU and few-GPU runs by checkpoint label alone. Use matched `effective_batch_size`, `seen_train_scenes`, or epoch checkpoints.")
    ...
    lines.append("- For active runs, wait for matched-exposure checkpoint PDMS before declaring an algorithm improvement or failure.")
    lines.append("- If matched-exposure GRPO variants remain below the historical strong baseline, the prepared step-level DPPO/GSPO path is the next grounded variant because it controls policy-ratio drift instead of relying only on offline targets.")
    lines.append("")
...
def main() -> None:
    ...
    parser.add_argument(
        "--baseline-pdms",
        type=float,
        default=0.9061843202874436,
        help="Long-horizon best Stage3/Safe DiffGRPO target.",
    )
    parser.add_argument(
        "--original-stage3-pdms",
        type=float,
        default=0.9055,
        help="Original Stage3 long-horizon reference obtained at epoch9-step13300.",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=800000,
        help="Approximate train-scene exposure required before direct long-horizon baseline comparison.",
    )
