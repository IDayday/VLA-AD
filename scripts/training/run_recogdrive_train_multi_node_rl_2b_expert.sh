...
: "${RECOGDRIVE_IL_CHECKPOINT:?Set RECOGDRIVE_IL_CHECKPOINT to the best IL checkpoint for RL initialization.}"
...
RECOGDRIVE_METRIC_CACHE_DIR=${RECOGDRIVE_METRIC_CACHE_DIR:-${NAVSIM_EXP_ROOT}/metric_cache_train}
...
LOG_FILE=${LOG_FILE:-${NAVSIM_EXP_ROOT}/train_recogdrive_${EXPERT_VARIANT}_expert_rl.txt}
...
torchrun \
  ...
  "agent.checkpoint_path=${RECOGDRIVE_IL_CHECKPOINT}" \
  "agent.dit_type=small" \
  "agent.vlm_size=small" \
  "agent.sampling_method=ddim" \
  "agent.metric_cache_path=${RECOGDRIVE_METRIC_CACHE_DIR}" \
  "agent.reference_policy_checkpoint=${RECOGDRIVE_IL_CHECKPOINT}" \
  ...
  "experiment_name=training_recogdrive_${EXPERT_VARIANT}_expert_rl" \
