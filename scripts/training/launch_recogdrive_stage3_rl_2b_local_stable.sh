...
EARLY_GATE_MIN_STOP_STEP="${EARLY_GATE_MIN_STOP_STEP:-3000}"
...
for name in \
  DIT_DROPOUT GRPO_DENOISED_CLIP_VALUE GRPO_EVAL_RANDN_CLIP_VALUE \
  GRPO_RANDN_CLIP_VALUE GRPO_FINAL_ACTION_CLIP_VALUE \
  GRPO_EVAL_MIN_SAMPLING_DENOISING_STD GRPO_MIN_SAMPLING_DENOISING_STD \
  GRPO_MIN_LOGPROB_DENOISING_STD GRPO_CLIP_ADVANTAGE_LOWER_QUANTILE \
  GRPO_CLIP_ADVANTAGE_UPPER_QUANTILE GRPO_GAMMA_DENOISING; do
  if [[ -n "${!name:-}" ]]; then
    JOB_CMD+="${name}=$(printf '%q' "${!name}") "
  fi
done
...
if [[ -n "${GRPO_ADVANTAGE_MODE:-}" ]]; then
  JOB_CMD+="GRPO_ADVANTAGE_MODE=$(printf '%q' "${GRPO_ADVANTAGE_MODE}") "
fi
...
for name in \
  GRPO_HARD_GATE_TTC GRPO_HARD_GATE_DDC GRPO_REWARD_MODE GRPO_SAFETY_ADVANTAGE_MODE \
  GRPO_TTC_SAFE_THRESHOLD GRPO_DDC_SAFE_THRESHOLD \
  GRPO_SOFT_SAFETY_PENALTY_WEIGHT GRPO_SOFT_SAFETY_PENALTY_CLIP \
  GRPO_SOFT_SAFETY_MIN_REWARD; do
  if [[ -n "${!name:-}" ]]; then
    JOB_CMD+="${name}=$(printf '%q' "${!name}") "
  fi
done
for name in \
  USE_TWO_EXPERT_SLOTS TWO_EXPERT_CACHE_MODE TWO_EXPERT_SLOT_MODE \
  TWO_EXPERT_CONDITION_MODE TWO_EXPERT_DIT_CONDITION_MODE \
  TWO_EXPERT_PLANNER_DIM TWO_EXPERT_USE_RAW_VLM_BASE TWO_EXPERT_ZERO_INIT_DELTAS \
  TWO_EXPERT_NUM_DYN_GROUPS TWO_EXPERT_DYN_TOKENS_PER_GROUP \
  TWO_EXPERT_NUM_GEO_TOKENS TWO_EXPERT_MEMORY_TOKENS_TO_DIT \
  TWO_EXPERT_MEMORY_SCALE_INIT TWO_EXPERT_PREFUSION_HEADS \
  TWO_EXPERT_PREFUSION_DROPOUT TWO_EXPERT_PREFUSION_TOKEN_DROPOUT \
  TWO_EXPERT_PREFUSION_CONDITION_DROPOUT TWO_EXPERT_PREFUSION_SCALE_INIT \
  TWO_EXPERT_PREFUSION_ZERO_INIT; do
  if [[ -n "${!name:-}" ]]; then
    JOB_CMD+="${name}=$(printf '%q' "${!name}") "
  fi
done
...
for name in \
  ...
  GRPO_BUFFER_PREFERENCE_DPO_LOSS_WEIGHT GRPO_BUFFER_PREFERENCE_DPO_LOSS_SCHEDULE \
  GRPO_BUFFER_PREFERENCE_DPO_LOSS_WEIGHT_START \
  GRPO_BUFFER_PREFERENCE_DPO_WARMUP_START_EPOCH \
  GRPO_BUFFER_PREFERENCE_DPO_WARMUP_EPOCHS \
  GRPO_BUFFER_PREFERENCE_DPO_TIMESTEP_SAMPLING GRPO_BUFFER_PREFERENCE_DPO_BETA \
  GRPO_BUFFER_PREFERENCE_DPO_LABEL_SMOOTHING GRPO_BUFFER_PREFERENCE_DPO_REFERENCE_FREE \
  GRPO_BUFFER_PREFERENCE_DPO_PAIR_MODE GRPO_BUFFER_PREFERENCE_DPO_MIN_REWARD_GAP \
  GRPO_BUFFER_PREFERENCE_DPO_MAX_PAIRS_PER_SCENE \
  GRPO_BUFFER_PREFERENCE_DPO_GAP_WEIGHT_MODE GRPO_BUFFER_PREFERENCE_DPO_GAP_WEIGHT_SCALE \
  GRPO_BUFFER_PREFERENCE_DPO_GAP_WEIGHT_MIN GRPO_BUFFER_PREFERENCE_DPO_GAP_WEIGHT_MAX \
  GRPO_BUFFER_PREFERENCE_DPO_INCLUDE_IL \
  ...
  GRPO_SELF_IMITATION_REQUIRE_NC GRPO_SELF_IMITATION_REQUIRE_DAC \
  GRPO_SELF_IMITATION_REQUIRE_TTC GRPO_SELF_IMITATION_REQUIRE_DDC \
  GRPO_SELF_IMITATION_NC_MIN_ABSOLUTE GRPO_SELF_IMITATION_DAC_MIN_ABSOLUTE \
  GRPO_SELF_IMITATION_TTC_MIN_ABSOLUTE GRPO_SELF_IMITATION_DDC_MIN_ABSOLUTE \
...

{
  ...
  echo "early_gate_rule=report if PDMS < threshold - margin; stop only when early_gate_stop_on_fail=1 and checkpoint_step >= early_gate_min_stop_step"
...
if [[ "${START_EARLY_GATE_WATCHER}" == "1" && "${DRY_RUN:-0}" != "1" ]]; then
  setsid env \
    ...
    MIN_STOP_STEP="${EARLY_GATE_MIN_STOP_STEP}" \
