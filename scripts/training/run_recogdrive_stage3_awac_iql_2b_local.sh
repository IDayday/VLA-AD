...
SUPPORT_ARCHIVE_PATH="${SUPPORT_ARCHIVE_PATH:-${ELITE_BUFFER_DIR}}"
...
STAGE3_CHECKPOINT_EVERY_N_EPOCHS="${STAGE3_CHECKPOINT_EVERY_N_EPOCHS:-${CHECKPOINT_EVERY_N_EPOCHS:-1}}"
STAGE3_CHECKPOINT_EVERY_N_TRAIN_STEPS="${STAGE3_CHECKPOINT_EVERY_N_TRAIN_STEPS:-${CHECKPOINT_EVERY_N_TRAIN_STEPS:-0}}"
...
SG_FPS_USE_DPSI="${SG_FPS_USE_DPSI:-false}"
SG_FPS_DPSI_TOP_M="${SG_FPS_DPSI_TOP_M:-${AWAC_ELITE_TOP_M}}"
SG_FPS_DPSI_WEIGHT_SAFE_EP="${SG_FPS_DPSI_WEIGHT_SAFE_EP:-1.0}"
SG_FPS_DPSI_WEIGHT_VECTOR_PARETO="${SG_FPS_DPSI_WEIGHT_VECTOR_PARETO:-0.9}"
SG_FPS_DPSI_WEIGHT_SAFETY_REPAIR="${SG_FPS_DPSI_WEIGHT_SAFETY_REPAIR:-0.8}"
SG_FPS_DPSI_WEIGHT_DDC_REPAIR="${SG_FPS_DPSI_WEIGHT_DDC_REPAIR:-0.8}"
SG_FPS_DPSI_WEIGHT_BEST_PDMS="${SG_FPS_DPSI_WEIGHT_BEST_PDMS:-0.7}"
SG_FPS_DPSI_WEIGHT_SMOOTH="${SG_FPS_DPSI_WEIGHT_SMOOTH:-0.6}"
SG_FPS_DPSI_WEIGHT_IL="${SG_FPS_DPSI_WEIGHT_IL:-0.5}"
SG_FPS_DPSI_WEIGHT_GT="${SG_FPS_DPSI_WEIGHT_GT:-0.4}"
SG_FPS_DPSI_PAIRWISE_RANK_WEIGHT="${SG_FPS_DPSI_PAIRWISE_RANK_WEIGHT:-0.0}"
SG_FPS_DPSI_PAIRWISE_BETA="${SG_FPS_DPSI_PAIRWISE_BETA:-8.0}"
USE_FS_NORM="${USE_FS_NORM:-false}"
FS_NORM_STATS_PATH="${FS_NORM_STATS_PATH:-}"
FS_NORM_USE_ROBUST="${FS_NORM_USE_ROBUST:-false}"
FS_NORM_CLIP="${FS_NORM_CLIP:-5.0}"
USE_PLANNING_TOKEN_ADAPTER="${USE_PLANNING_TOKEN_ADAPTER:-false}"
PLANNING_TOKEN_SOURCE="${PLANNING_TOKEN_SOURCE:-none}"
PLANNING_NUM_TOKENS="${PLANNING_NUM_TOKENS:-16}"
PLANNING_NUM_HEADS="${PLANNING_NUM_HEADS:-8}"
PLANNING_CONDITION_LAYERS="${PLANNING_CONDITION_LAYERS:-cross_attention}"
PLANNING_GATE_INIT="${PLANNING_GATE_INIT:-0.05}"
PLANNING_CONTEXT_GATE_INIT="${PLANNING_CONTEXT_GATE_INIT:-0.05}"
PLANNING_CONDITION_DROPOUT="${PLANNING_CONDITION_DROPOUT:-0.10}"
X0_AUX_WEIGHT="${X0_AUX_WEIGHT:-0.0}"
DELTA_AUX_WEIGHT="${DELTA_AUX_WEIGHT:-0.0}"
GEO_AUX_WEIGHT="${GEO_AUX_WEIGHT:-0.0}"
TRAJECTORY_AUX_WEIGHT="${TRAJECTORY_AUX_WEIGHT:-0.0}"
FEASIBILITY_AUX_WEIGHT="${FEASIBILITY_AUX_WEIGHT:-0.0}"
AUX_ALPHA_POWER="${AUX_ALPHA_POWER:-1.0}"
AUX_WARMUP_EPOCHS="${AUX_WARMUP_EPOCHS:-10}"
TANGENT_MARGIN_RAD="${TANGENT_MARGIN_RAD:-0.08}"
CURVATURE_MARGIN="${CURVATURE_MARGIN:-0.05}"
MIN_SEGMENT_LENGTH="${MIN_SEGMENT_LENGTH:-0.20}"
X0_AUX_LOW_NOISE_FRAC="${X0_AUX_LOW_NOISE_FRAC:-0.5}"
GEO_CURVATURE_WEIGHT="${GEO_CURVATURE_WEIGHT:-1.0}"
GEO_REVERSE_WEIGHT="${GEO_REVERSE_WEIGHT:-1.0}"
GEO_TAIL_REVERSE_WEIGHT="${GEO_TAIL_REVERSE_WEIGHT:-2.0}"
GEO_EARLY_KINK_WEIGHT="${GEO_EARLY_KINK_WEIGHT:-2.0}"
GEO_JERK_WEIGHT="${GEO_JERK_WEIGHT:-0.2}"
...
if [[ "${DRY_RUN}" != "1" ]]; then
  ...
  if [[ "${USE_FS_NORM}" == "true" && ! -f "${FS_NORM_STATS_PATH}" ]]; then
    echo "FS_NORM_STATS_PATH must exist when USE_FS_NORM=true: ${FS_NORM_STATS_PATH}" >&2
    exit 2
  fi
fi
...
HYDRA_ARGS=(
  ...
  "agent.offline_rl_use_dpsi=${SG_FPS_USE_DPSI}"
  "agent.offline_rl_dpsi_top_m=${SG_FPS_DPSI_TOP_M}"
  "agent.offline_rl_dpsi_weight_safe_ep=${SG_FPS_DPSI_WEIGHT_SAFE_EP}"
  "agent.offline_rl_dpsi_weight_vector_pareto=${SG_FPS_DPSI_WEIGHT_VECTOR_PARETO}"
  "agent.offline_rl_dpsi_weight_safety_repair=${SG_FPS_DPSI_WEIGHT_SAFETY_REPAIR}"
  "agent.offline_rl_dpsi_weight_ddc_repair=${SG_FPS_DPSI_WEIGHT_DDC_REPAIR}"
  "agent.offline_rl_dpsi_weight_best_pdms=${SG_FPS_DPSI_WEIGHT_BEST_PDMS}"
  "agent.offline_rl_dpsi_weight_smooth=${SG_FPS_DPSI_WEIGHT_SMOOTH}"
  "agent.offline_rl_dpsi_weight_il=${SG_FPS_DPSI_WEIGHT_IL}"
  "agent.offline_rl_dpsi_weight_gt=${SG_FPS_DPSI_WEIGHT_GT}"
  "agent.offline_rl_dpsi_pairwise_rank_weight=${SG_FPS_DPSI_PAIRWISE_RANK_WEIGHT}"
  "agent.offline_rl_dpsi_pairwise_beta=${SG_FPS_DPSI_PAIRWISE_BETA}"
  ...
  "agent.use_fs_norm=${USE_FS_NORM}"
  "agent.fs_norm_stats_path=${FS_NORM_STATS_PATH}"
  "agent.fs_norm_use_robust=${FS_NORM_USE_ROBUST}"
  "agent.fs_norm_clip=${FS_NORM_CLIP}"
  "agent.use_planning_token_adapter=${USE_PLANNING_TOKEN_ADAPTER}"
  "agent.planning_token_source=${PLANNING_TOKEN_SOURCE}"
  "agent.planning_num_tokens=${PLANNING_NUM_TOKENS}"
  "agent.planning_num_heads=${PLANNING_NUM_HEADS}"
  "agent.planning_condition_layers=${PLANNING_CONDITION_LAYERS}"
  "agent.planning_gate_init=${PLANNING_GATE_INIT}"
  "agent.planning_context_gate_init=${PLANNING_CONTEXT_GATE_INIT}"
  "agent.planning_condition_dropout=${PLANNING_CONDITION_DROPOUT}"
  "agent.x0_aux_weight=${X0_AUX_WEIGHT}"
  "agent.delta_aux_weight=${DELTA_AUX_WEIGHT}"
  "agent.geo_aux_weight=${GEO_AUX_WEIGHT}"
  "agent.trajectory_aux_weight=${TRAJECTORY_AUX_WEIGHT}"
  "agent.feasibility_aux_weight=${FEASIBILITY_AUX_WEIGHT}"
  "agent.aux_alpha_power=${AUX_ALPHA_POWER}"
  "agent.aux_warmup_epochs=${AUX_WARMUP_EPOCHS}"
  "agent.tangent_margin_rad=${TANGENT_MARGIN_RAD}"
  "agent.curvature_margin=${CURVATURE_MARGIN}"
  "agent.min_segment_length=${MIN_SEGMENT_LENGTH}"
  "agent.x0_aux_low_noise_frac=${X0_AUX_LOW_NOISE_FRAC}"
  "agent.geo_curvature_weight=${GEO_CURVATURE_WEIGHT}"
  "agent.geo_reverse_weight=${GEO_REVERSE_WEIGHT}"
  "agent.geo_tail_reverse_weight=${GEO_TAIL_REVERSE_WEIGHT}"
  "agent.geo_early_kink_weight=${GEO_EARLY_KINK_WEIGHT}"
  "agent.geo_jerk_weight=${GEO_JERK_WEIGHT}"
...

{
  ...
  echo "checkpoint_every_n_epochs=${STAGE3_CHECKPOINT_EVERY_N_EPOCHS}"
  echo "checkpoint_every_n_train_steps=${STAGE3_CHECKPOINT_EVERY_N_TRAIN_STEPS}"
  ...
  echo "sg_fps_use_dpsi=${SG_FPS_USE_DPSI}"
  echo "sg_fps_dpsi_top_m=${SG_FPS_DPSI_TOP_M}"
  echo "sg_fps_dpsi_weight_safe_ep=${SG_FPS_DPSI_WEIGHT_SAFE_EP}"
  echo "sg_fps_dpsi_weight_vector_pareto=${SG_FPS_DPSI_WEIGHT_VECTOR_PARETO}"
  echo "sg_fps_dpsi_weight_safety_repair=${SG_FPS_DPSI_WEIGHT_SAFETY_REPAIR}"
  echo "sg_fps_dpsi_weight_ddc_repair=${SG_FPS_DPSI_WEIGHT_DDC_REPAIR}"
  echo "sg_fps_dpsi_weight_best_pdms=${SG_FPS_DPSI_WEIGHT_BEST_PDMS}"
  echo "sg_fps_dpsi_weight_smooth=${SG_FPS_DPSI_WEIGHT_SMOOTH}"
  echo "sg_fps_dpsi_weight_il=${SG_FPS_DPSI_WEIGHT_IL}"
  echo "sg_fps_dpsi_weight_gt=${SG_FPS_DPSI_WEIGHT_GT}"
  echo "sg_fps_dpsi_pairwise_rank_weight=${SG_FPS_DPSI_PAIRWISE_RANK_WEIGHT}"
  echo "sg_fps_dpsi_pairwise_beta=${SG_FPS_DPSI_PAIRWISE_BETA}"
  echo "use_fs_norm=${USE_FS_NORM}"
  echo "fs_norm_stats_path=${FS_NORM_STATS_PATH}"
  echo "fs_norm_use_robust=${FS_NORM_USE_ROBUST}"
  echo "fs_norm_clip=${FS_NORM_CLIP}"
  echo "use_planning_token_adapter=${USE_PLANNING_TOKEN_ADAPTER}"
  echo "planning_token_source=${PLANNING_TOKEN_SOURCE}"
  echo "planning_num_tokens=${PLANNING_NUM_TOKENS}"
  echo "planning_num_heads=${PLANNING_NUM_HEADS}"
  echo "planning_condition_layers=${PLANNING_CONDITION_LAYERS}"
  echo "planning_gate_init=${PLANNING_GATE_INIT}"
  echo "planning_context_gate_init=${PLANNING_CONTEXT_GATE_INIT}"
  echo "planning_condition_dropout=${PLANNING_CONDITION_DROPOUT}"
  echo "x0_aux_weight=${X0_AUX_WEIGHT}"
  echo "delta_aux_weight=${DELTA_AUX_WEIGHT}"
  echo "geo_aux_weight=${GEO_AUX_WEIGHT}"
  echo "trajectory_aux_weight=${TRAJECTORY_AUX_WEIGHT}"
  echo "feasibility_aux_weight=${FEASIBILITY_AUX_WEIGHT}"
  echo "aux_alpha_power=${AUX_ALPHA_POWER}"
  echo "aux_warmup_epochs=${AUX_WARMUP_EPOCHS}"
  echo "tangent_margin_rad=${TANGENT_MARGIN_RAD}"
  echo "curvature_margin=${CURVATURE_MARGIN}"
  echo "min_segment_length=${MIN_SEGMENT_LENGTH}"
  echo "x0_aux_low_noise_frac=${X0_AUX_LOW_NOISE_FRAC}"
  echo "geo_curvature_weight=${GEO_CURVATURE_WEIGHT}"
  echo "geo_reverse_weight=${GEO_REVERSE_WEIGHT}"
  echo "geo_tail_reverse_weight=${GEO_TAIL_REVERSE_WEIGHT}"
  echo "geo_early_kink_weight=${GEO_EARLY_KINK_WEIGHT}"
  echo "geo_jerk_weight=${GEO_JERK_WEIGHT}"
