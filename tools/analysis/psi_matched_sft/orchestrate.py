...
def launch_evaluation(name,step,gpu,rank,host):
    cmd=[PY,str(CODE/'evaluate.py'),name,'--step',str(step),'--rank',str(rank),'--world','3']
    if host:
...
def main():
    ...
    for gpu,(method,runseed) in enumerate((m,s) for m in CFG['methods'] for s in CFG['train_seeds']):
        name=f'{method}_seed{runseed}'
        for rank,host in enumerate([None,'training-rl-zt4','training-vla-zt2']):jobs.append(launch_evaluation(name,512,gpu,rank,host))
    for p in jobs:
        if p.wait():raise RuntimeError('Final evaluation failed')
    jobs=[]
    for gpu,(method,runseed) in enumerate((m,s) for m in CFG['methods'] for s in CFG['train_seeds']):
        name=f'{method}_seed{runseed}'
        for rank,host in enumerate([None,'training-rl-zt4','training-vla-zt2']):jobs.append(launch_evaluation(name,128,gpu,rank,host))
    for p in jobs:
