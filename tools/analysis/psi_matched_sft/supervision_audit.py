    ...
    for s in scenes():
        ...
        for method,p in selection.items():
            for idx,w in zip(p['indices'],p['weights']):
                rows.append(dict(token=t,split=s['split'],method=method,raw_index=idx,weight=w,trajectory=tr[idx].reshape(-1).tolist(),trajectory_sha256=digest(tr[idx].tolist()),all_source_tags=meta['all_source_tags'][idx],archive_path=meta['archive_path'],archive_sha256=meta['archive_sha256'],archive_indices=meta['raw_indices'][idx],PDMS=sc[idx,6]*100,feasible=bool(safe(sc[idx])),selection_sha256=frozen[t]))
